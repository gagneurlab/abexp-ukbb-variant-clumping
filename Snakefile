...
#Downloads GWAS-results from the UK-Biobank for a given trait. The filename of the GWAS-results file is specified in the config.
rule download_GWAS_results:
    output:
        "{dir}sumstats.tsv.gz"
    shell:
        f"wget -q -O {{output}} 'https://pan-ukb-us-east-1.s3.amazonaws.com/sumstats_flat_files/{config['filename']}'"
