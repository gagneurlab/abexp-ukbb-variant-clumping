...
if "pval_meta" in col_names:
    ...
    df = pd.read_csv(sys.argv[1], sep='\t', usecols=["chr", "pos", "ref", "alt", "neglog10_pval_meta"] , dtype={"chr":str, "pos":int, "ref":str, "alt":str, "neglog10_pval_meta":float})
    #Convert from neglog to normal values:
    df["pval_meta_raw"] = 10 ** ((-1) * df["neglog10_pval_meta"])
else:
    print(f"The {sys.argv[1]} file doesn't contain any of the expected pvalue columns.")
    sys.exit()
